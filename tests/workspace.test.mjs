...
assert.equal(store.papers.length, 0)
assert.equal(store.documents.length, 0)
for (const id of [1, 2]) {
  store.papers.push({id, title: 'Test paper', noteIds: [], tags: [], status: 'to_read'})
  store.documents.push({id, slug: 'note-' + id, title: 'Test note', summary: '', excerpt: '', content: '', tags: [], kind: 'Research Note', visibility: 'public', publishedAt: '2026-01-01', updatedAt: '2026-01-01', trashedAt: null})
}
store.publications.push({id: 1, title: 'Test publication', tags: []})
...
const metadata = JSON.parse(JSON.stringify(store.documents[0])); metadata.summary = 'Keep the summary'; metadata.excerpt = 'Keep the separate excerpt'; metadata.content = '# Replaced body'; await store.saveDocument(metadata)
assert.equal(store.documents[0].summary, 'Keep the summary'); assert.equal(store.documents[0].excerpt, 'Keep the separate excerpt')
console.log('PASS replacing content preserves independently written summary and excerpt')
...
assert.equal(reloaded.papers.length, 0, 'Do not load old browser records before the API')
