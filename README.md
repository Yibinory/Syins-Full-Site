...
- Recommended papers with reading status, tags, export and zero-to-many Notes links
- Markdown Documents with public/private/unlisted visibility and recoverable Trash
- Server inventory with capability snapshots and audited, whitelisted actions
...
For the database and service boundaries, see [ARCHITECTURE.md](ARCHITECTURE.md). For the detailed backend and database design, see [BACKEND_DATABASE_DESIGN.md](BACKEND_DATABASE_DESIGN.md).
...
- Server operations use an explicit action union and are recorded in `ServerActionLog`; arbitrary shell execution is intentionally unavailable.
...
The frontend tests cover interactive HTML/Vue/ZIP import, archive safety and workspace relation behavior. Backend tests cover public/private permissions, authentication, paper–Note relations, tag merging, media upload and safe server actions.
