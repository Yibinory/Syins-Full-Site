...
<template><figure class="research-visual project-media"><div class="project-media-frame" :class="{ 'interactive-frame': project.mediaType === 'interactive' }"><p v-if="loading" class="project-media-empty">{{ $t("Loading visual…") }}</p><p v-else-if="failed || (!url && !html)" class="project-media-empty">{{ $t("Research figure coming soon") }}</p><iframe v-else-if="project.mediaType === 'interactive'" :srcdoc="html" :title="project.mediaAlt || project.title" sandbox="allow-scripts" referrerpolicy="no-referrer" /><video v-else-if="project.mediaType === 'video'" :key="url" :src="url" :aria-label="project.mediaAlt || project.title" controls playsinline preload="metadata" @error="failed = true" /><img v-else :src="url" :alt="project.mediaAlt || project.title" loading="lazy" @error="failed = true" /></div><figcaption v-if="project.caption">{{ project.caption }}</figcaption></figure></template>
