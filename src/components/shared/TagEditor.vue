...
<template><div class="tag-editor"><span class="field-label">{{ label || $t('Tags') }}</span><div class="editable-tags"><span v-for="(tag, index) in modelValue" :key="tag"><input :value="tag" :aria-label="`Rename tag ${tag}`" @change="rename(index, $event)" /><button type="button" :aria-label="`Remove tag ${tag}`" @click="emit('update:modelValue', modelValue.filter((_, i) => i !== index))">×</button></span></div><div class="inline-controls"><input v-model="draft" :placeholder="$t('Add tags, separated by commas')" :aria-label="$t('New tags')" @keydown.enter.prevent="add" /><button type="button" class="text-action" @click="add">{{ $t("Add tags") }}</button></div></div></template>
