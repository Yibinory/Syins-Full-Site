...
import { BookOpenText, Boxes, ChevronLeft, ExternalLink, FileText, Gauge, LayoutTemplate, Newspaper, Menu, Network, PanelLeftClose, PanelLeftOpen, Settings, Tags, X } from 'lucide-vue-next'
...
<template>
  <div class="dashboard-shell" :class="{ 'is-collapsed': collapsed, 'nav-open': open }">
    ...
    <div class="dashboard-main">
      <header class="dashboard-topbar"><LanguageSwitch /><button class="mobile-dashboard-menu" :aria-label="$t('Open navigation')" @click="open = true"><Menu :size="19" /></button><button class="collapse-button" :aria-label="$t(collapsed ? 'Expand sidebar' : 'Collapse sidebar')" :aria-expanded="!collapsed" @click="collapsed = !collapsed"><component :is="collapsed ? PanelLeftOpen : PanelLeftClose" :size="17" /></button><span class="sync-state"><i /> {{ $t("Connected workspace") }}</span></header>
