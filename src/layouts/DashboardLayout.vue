...
<template>
  <div class="dashboard-shell" :class="{ 'is-collapsed': collapsed, 'nav-open': open }">
    <aside class="dashboard-sidebar">
      <div class="dashboard-brand"><div class="brand-mark">{{ $t("SY") }}</div><div class="brand-text"><strong>{{ $t("Research OS") }}</strong><span>{{ $t("Syins Yibinory") }}</span></div><button :aria-label="$t('Close navigation')" @click="open = false"><X :size="18" /></button></div>
      ...
      <div class="sidebar-footer"><RouterLink to="/"><ChevronLeft :size="16" /><span>{{ $t("Public site") }}</span></RouterLink><button @click="logout"><span class="user-avatar">{{ $t("SY") }}</span><span class="user-copy"><strong>{{ $t("Syins Yibinory") }}</strong><small>{{ $t("Sign out") }}</small></span></button></div>
    </aside>
