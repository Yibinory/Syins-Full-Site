...
const grouped = computed(() => {
  ...
  const groups = items.reduce<Record<number, typeof publications.value>>((groups, paper) => {
    ;(groups[paper.year] ??= []).push(paper)
    return groups
  }, {})
  return Object.entries(groups).sort(([a], [b]) => Number(b) - Number(a)).map(([year, papers]) => ({ year, papers }))
})
</script>
...
<template>
  <PublicLayout>
    ...
    <section class="publication-browser page-grid">
      ...
      <div class="publication-years"><p v-if="copyNotice" role="status">{{ $t(copyNotice) }}</p><p v-if="!grouped.length">{{ $t("No publications in this category.") }}</p>
