...
<template>
  <PublicLayout>
    <header class="public-page-intro publications-intro page-grid">
      <p class="section-kicker">Publications</p><h1>{{ siteContent.content.publicationsHeading }}</h1><p>{{ siteContent.content.publicationsDescription }}</p>
    </header>
    <section class="publication-browser page-grid">
      <div class="publication-filters" aria-label="Filter publications"><button v-for="filter in filters" :key="filter" :class="{ active: active === filter }" @click="active = filter">{{ filter }}</button></div>
      <div class="publication-years"><p v-if="copyNotice" role="status">{{ copyNotice }}</p><p v-if="!Object.keys(grouped).length">No publications in this category.</p>
        <section v-for="(papers, year) in grouped" :key="year" class="publication-year-group"><h2>{{ year }}</h2><article v-for="paper in papers" :key="paper.id" class="publication-entry"><ResearchMedia v-if="paper.mediaAssetId || paper.mediaUrl" :project="paper" /><div v-else class="publication-thumb"><span>{{ paper.venueShort }}</span></div><div class="publication-entry-copy"><p class="pub-venue">{{ paper.venueShort }} · {{ paper.type }}</p><h3>{{ paper.title }}</h3><p class="pub-authors">{{ paper.authors }}</p><p v-if="paper.motivation" class="publication-motivation">{{ paper.motivation }}</p><p v-if="paper.approach">{{ paper.approach }}</p><details v-if="paper.abstract"><summary>Abstract</summary><p>{{ paper.abstract }}</p></details><div class="tag-list"><span v-for="tag in paper.tags" :key="tag">{{ tag }}</span></div><div class="publication-actions"><a v-if="paper.paperUrl" :href="paper.paperUrl">Paper <ArrowUpRight :size="14" /></a><a v-if="paper.codeUrl" :href="paper.codeUrl">Code <ArrowUpRight :size="14" /></a><a v-if="paper.projectUrl" :href="paper.projectUrl">Project <ArrowUpRight :size="14" /></a><button v-if="paper.bibtex" @click="copyBibtex(paper.bibtex)"><Copy :size="13" /> BibTeX</button></div></div></article></section>
      </div>
    </section>
  </PublicLayout>
</template>
