  ...
  <div>
    ...
    <section class="server-grid">
      <article v-for="server in visibleServers" :key="server.id" class="server-card" :class="{ 'is-offline': server.status === 'offline', 'is-primary': server.id === selectedServer?.id }" @click="selectedServerId = server.id">
        ...
        <template v-if="server.lastSeen || server.provider === 'mock'"><div class="resource-bars"><div><span>{{ $t("CPU") }} <b>{{ server.cpu }}%</b></span><i><em :style="{ width: barWidth(server.cpu) }" /></i></div><div><span>{{ $t("RAM") }} <b>{{ server.memory.used }} / {{ server.memory.total }} {{ $t("GiB") }}</b></span><i><em :style="{ width: barWidth(server.memory.total ? server.memory.used / server.memory.total * 100 : 0) }" /></i></div><div><span>{{ $t("Disk") }} <b>{{ server.disk.used }} / {{ server.disk.total }} {{ $t("TiB") }}</b></span><i><em :style="{ width: barWidth(server.disk.total ? server.disk.used / server.disk.total * 100 : 0) }" /></i></div></div><div v-if="server.gpus.length" class="gpu-strip"><div class="gpu-heading"><span><Cpu :size="14" /> {{ server.gpus[0].name }} × {{ server.gpus.length }}</span><small>{{ $t("GPU utilization") }}</small></div><div class="gpu-values"><span v-for="(gpu, index) in server.gpus" :key="index"><b>{{ gpu.utilization }}</b><small>%</small></span></div></div><div v-else class="no-gpu"><Cpu :size="15" /><span>{{ $t("No NVIDIA GPU reported") }}</span></div></template>
